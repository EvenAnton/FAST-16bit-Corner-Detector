...
std::vector<int> fast_16bit_corner_detector::fast9_score(cv::Mat im, std::vector<xy> corners)
{	
  ...
  for(xy c : corners) {
    ...
    scores.push_back(fast9_corner_score(p, pixel));
  }
	return scores;
}


std::vector<fast_16bit_corner_detector::xy> fast_16bit_corner_detector::fast9_detect(const cv::Mat im)
{
  int b = threshold;
	int num_corners=0;
